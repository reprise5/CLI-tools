...
while [ 1 ]; do
	...
	#-- exit code 3                    user hit back button
	#-- exit code 0 and blank  choice: User cancelled.
	#-- exit code 0 and filled choice: user wants to continue, so move on (break for right now)
	if [ $? -eq 3 ]; then
		show_home
		choices=""
	elif [[ ! -z "$choices" ]]; then
